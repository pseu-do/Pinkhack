//----Congrats you found a old resolver :) ----//
...
float CResolver::ResolveYaw(CBaseEntity* pPlayer, float org_yaw)
{
    ...
    bool is_moving = pPlayer->GetVelocity().Length() > 20.f;

    bool is_lowerbodyupdate = is_moving && is_onground;

    bool is_fakewalking = pPlayer->GetVelocity().Length() < 20.f && pPlayer->GetVelocity().Length() > 1.f; //ghetto fakewalk


    auto cur = this->m_arrInfos.at(index).m_sRecords;

    if (is_lowerbodyupdate)
    {
        LatestLowerBodyYaw[index] = pPlayer->LowerBodyYaw();

        YawDifference[index] = Math::NormalizeYaw(new_yaw - pPlayer->LowerBodyYaw());

        OldLbyDelta[index] = Math::NormalizeYaw(pPlayer->GetEyeAngles().y - pPlayer->LowerBodyYaw());
    ...
    else if (is_fakewalking)
    {
        ...
        else
        {
            ...
            new_yaw = OldLbyDelta[index];
            aa_info[index] = "Old Lby Delta";
        }
    }
    else /*On ground resolver*/
    {
        ...
        {
            if (((g_pGlobals->curtime - LatestLowerBodyYawUpdateTime[index]) >= 1.1f))
            {
                new_yaw = pPlayer->LowerBodyYaw();
                aa_info[index] = "Predict Update";
            }
            else
            {
                // do all other logic here
